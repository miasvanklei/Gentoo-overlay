...
PDEPEND="
	pipewire? ( media-sound/pipewire-module-xrdp )
"
...
src_configure() {
	...
	local myconf=(
		...
		$(use pam || echo --enable-nopam)
		$(usex pam --enable-pam-config=arch '')
...
pkg_preinst() {
	...
	if [[ -f "${EROOT}"/etc/xrdp/rsakeys.ini ]]; then
		...
		LD_LIBRARY_PATH="${ED}/usr/lib/xrdp/" "${ED}"/usr/bin/xrdp-keygen xrdp "${ED}"/etc/xrdp/rsakeys.ini \
