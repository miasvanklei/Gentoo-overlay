...
inherit check-reqs cmake-utils eutils flag-o-matic multilib \
	multilib-minimal python-single-r1 toolchain-funcs pax-utils git-r3 prefix
...
src_unpack() {
	...
	if use lld; then
		git-r3_fetch "http://llvm.org/git/lld.git
        ...
        if use lld; then
                git-r3_checkout http://llvm.org/git/lld.git \
                        "${S}"/tools/lld
        fi
